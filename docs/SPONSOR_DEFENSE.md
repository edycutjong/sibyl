# OracleChain — Sponsor Defense Brief

## Why ONLY Prophet Arena?

OracleChain is built **exclusively** for the Prophet Arena evaluation ecosystem. Every component integrates with Prophet Arena's toolchain. Remove Prophet Arena and you'd need to build 5 separate systems from scratch.
...
| 3 | `prophet forecast events` | Lists open/closed events from **live server** (requires `PA_SERVER_API_KEY`) | `oraclechain/agent.py` |
| 4 | `prophet forecast predict --local` | Runs our agent against events using the standard `predict(event)` contract | `oraclechain/agent.py` |
| 5 | `prophet forecast predict --agent-url` | Tests our HTTP `/predict` endpoint via CLI | `oraclechain/server.py` |
...
| 10 | `ai_prophet_core.forecast.schemas.Prediction` | Dual format: `p_yes` (binary) OR `probabilities` list (multi-outcome) | `oraclechain/agent.py` |
| 11 | `ai_prophet.search.SearchClient` | **SDK built-in retrieval** — Exa, Brave, Tavily, Perplexity with sandbox mode | `oraclechain/retrieval.py` |
| 12 | OpenAI-compatible `POST /chat/completions` | HTTP endpoint contract — Prophet Arena calls this for auto-evaluation | `oraclechain/server.py` |
| 13 | `POST /predict` endpoint | CLI-compatible endpoint — `prophet forecast predict --agent-url` | `oraclechain/server.py` |
